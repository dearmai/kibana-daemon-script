...
PROCESS_KEYWORD="bin/../node/bin/node"
...
function init() { 
    ...
    if [ -f "${TARGET_PATH}/bin/kibana" ]; then
        return 0
    else
        print_err "스크립트 파일은 kibana 하위 폴더 내 설치 되어야 합니다."
        return 1
    fi
}
...
case $1 in
    status)
        ...
        echo "Network port status(${KB_PORT}) : $(check_netstat)"
        ;;
    run)
        print "Run"
        cd ${TARGET_PATH}
    ...
    start)
        ...
        cd ${TARGET_PATH}
        eval nohup "bin/kibana" > /dev/null 2>&1 "&"
        echo $! > ${PID_PATH}
